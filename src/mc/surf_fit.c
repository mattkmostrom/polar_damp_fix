...
int alloc_curves(int nCurves, int nPoints, curveData_t *curve) {
    ...
    if (!memSuccessFlag) {
        error(
            "SURFACE: Exhausted memory while allocating input/output/global arrays.\n");
...
void new_global_min(system_t *system, int nCurves, int nPoints, curveData_t *curve) {
    ...
    write_molecules_wrapper(system,
                            "fit_geometry.pqr");
    for (j = 0; j < nCurves; j++)
        for (i = 0; i < nPoints; i++)
...
void free_all_mem(int nCurves, curveData_t *curve, param_g *params, qshiftData_t *qshiftData, double *r_input) {
    int i;
